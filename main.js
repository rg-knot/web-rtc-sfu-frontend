...
// MEDIASOUP VANILLA JS CLIENT (main.js)
// ============================================
// Make sure these are loaded in HTML BEFORE this script:
...
class MediasoupClient {
    constructor(serverUrl) {
        ...
        // IMPORTANT: Access Device from global mediasoupClient
        this.device = null; // Will be initialized when joining room
        this.roomId = null;
        ...
        console.log('✅ MediasoupClient initialized');
    }
        ...
        return new Promise((resolve, reject) => {
            this.socket.emit('joinRoom', { roomId }, async (response) => {
                try {
                    ...
                    // CRITICAL FIX: Create Device from global mediasoupClient object
                    // Check if mediasoupClient is available globally
                    if (typeof mediasoupClient === 'undefined') {
                        throw new Error('mediasoupClient library not loaded. Check if script is included in HTML.');
                    }
    ...
    async createSendTransport() {
        return new Promise((resolve, reject) => {
            this.socket.emit(
                ...
                async (response) => {
                    try {
                        // Create send transport from device
                        this.sendTransport = this.device.createSendTransport({
                            id: response.id,
                            iceParameters: response.iceParameters,
                            iceCandidates: response.iceCandidates,
    ...
    async createRecvTransport() {
        return new Promise((resolve, reject) => {
            this.socket.emit(
                ...
                async (response) => {
                    try {
                        // Create receive transport from device
                        this.recvTransport = this.device.createRecvTransport({
                            id: response.id,
                            iceParameters: response.iceParameters,
                            iceCandidates: response.iceCandidates,
...
// Global variables
let mediasoupClientInstance = null;
let localStream = null;
let currentUsername = null;
let currentCallPeerId = null;
let isInCall = false;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof mediasoupClient === 'undefined') {
        ...
        alert('MediaSoup library failed to load. Please check your internet connection and refresh the page.');
        return;
    }
    
    if (typeof io === 'undefined') {
        console.error('❌ Socket.IO library not loaded!');
        alert('Socket.IO library failed to load. Please check your internet connection and refresh the page.');
        return;
    }
    
    console.log('✅ All libraries loaded successfully');
    console.log('   MediaSoup version:', mediasoupClient.version);
    console.log('   Device constructor:', typeof mediasoupClient.Device);
    
    // Initialize MediaSoup client
    const SERVER_URL = 'http://localhost:3000';
    mediasoupClientInstance = new MediasoupClient(SERVER_URL);
});
...
createUserBtn.addEventListener('click', async () => {
    ...
    mediasoupClientInstance.socket.emit('registerUser', { username }, (response) => {
        console.log('✅ User registered:', response);
    });
    
    mediasoupClientInstance.socket.on('userList', (users) => {
        updateUserList(users);
    });
});
...
function updateUserList(users) {
    ...
    users.forEach(user => {
        ...
        const span = document.createElement('span');
        span.textContent = user.username + (user.id === mediasoupClientInstance.socket.id ? ' (You)' : '');
        
        const button = document.createElement('button');
        button.className = 'call-btn';
        button.innerHTML = '<img width="20" src="./images/phone.png" alt="Call">';
        
        if (user.id === mediasoupClientInstance.socket.id) {
            button.disabled = true;
            button.style.opacity = '0.5';
        } else {
            button.addEventListener('click', () => startCall(user.id, user.username));
...
async function startCall(peerId, peerUsername) {
    ...
    try {
        ...
        // Step 1: Join room
        await mediasoupClientInstance.joinRoom(roomId);
        
        // Step 2: Create transports
        await mediasoupClientInstance.createTransports();
        ...
        // Step 4: Produce video and audio
        const videoTrack = localStream.getVideoTracks()[0];
        const audioTrack = localStream.getAudioTracks()[0];
        
        await mediasoupClientInstance.produce(videoTrack, 'video');
        await mediasoupClientInstance.produce(audioTrack, 'audio');
        
        console.log('✅ Producing video and audio');
        
        // Step 5: Setup consumer callback
        mediasoupClientInstance.onNewConsumer = (consumer, peerId) => {
            console.log(`📺 Received ${consumer.kind} from ${peerId}`);
        ...
        // Notify peer
        mediasoupClientInstance.socket.emit('callUser', { 
            targetUserId: peerId, 
            roomId 
        });
...
function endCall() {
    ...
    mediasoupClientInstance.cleanup();
...
// Handle incoming call
if (mediasoupClientInstance) {
    mediasoupClientInstance.socket.on('incomingCall', async ({ callerId, callerUsername, roomId }) => {
        if (isInCall) {
            mediasoupClientInstance.socket.emit('callRejected', { callerId, reason: 'busy' });
            return;
        }
        ...
        if (accept) {
            try {
                ...
                mediasoupClientInstance.socket.emit('callAccepted', { callerId });
            ...
            mediasoupClientInstance.socket.emit('callRejected', { callerId, reason: 'declined' });
        }
    });

    mediasoupClientInstance.socket.on('callEnded', () => {
        alert('Call ended by the other user');
        endCall();
    });
}
...
window.addEventListener('beforeunload', () => {
    ...
    if (mediasoupClientInstance) {
        mediasoupClientInstance.disconnect();
    }
});

// Expose for debugging
window.mediasoupDebug = {
    client: mediasoupClientInstance,
    device: () => mediasoupClientInstance?.device,
    startCall,
    endCall,
};